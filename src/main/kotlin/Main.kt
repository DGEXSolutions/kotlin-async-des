...
fun main(args: Array<String>) = runSimulation(0) {
    runBlocking {  }
    coroutineScope {
        launch {
            delay(1000)
            println("first launch ($currentTimeMillis)")
        }

        launch {
            delay(1000)
            println("second launch a ($currentTimeMillis)")
            delay(2000)
            println("second launch b ($currentTimeMillis)")
        }
    }
    println("ok")
}
